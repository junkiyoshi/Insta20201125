	...
	for (int x = -450; x <= 450; x += 450) {
		...
		for (int y = -450; y <= 450; y += 450) {
			...
			for (int radius = 70; radius <= 150; radius += 10) {
				...
				this->setRingToMesh(this->face, this->frame, glm::vec3(x, y, 0), radius, 5, 30, 0, 360);
			}
		}
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height, int deg_start, int deg_end) {
	...
	auto angle_x = ofMap(ofNoise(location.x, radius * 0.0012 + ofGetFrameNum() * 0.003), 0, 1, PI * -2, PI * 2);
	auto rotation_x = glm::rotate(glm::mat4(), angle_x, glm::vec3(1, 0, 0));

	auto angle_y = ofMap(ofNoise(location.y, radius * 0.0012 + ofGetFrameNum() * 0.003), 0, 1, PI * -2, PI * 2);
